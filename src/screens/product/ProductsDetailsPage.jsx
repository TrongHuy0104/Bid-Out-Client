...
export const ProductsDetailsPage = () => {
    ...
    }, [dispatch, id, product?.isSoldout]);

    useEffect(() => {
        if (history && history.length > 0) {
            const hightestBid = Math.max(...history.map((bid) => bid.price));
            setRate(hightestBid);
        } else if (product) {
    ...
    const save = async (e) => {
        e.preventDefault();
        if (product?.price > Number(rate)) {
            return toast.error(
                'Your bid must be greater than or equal to the product price'
            );
        }
